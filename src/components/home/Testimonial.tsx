...
const Testimonial: React.FC = () => {
    return (
        <section aria-label="Client Testimonials" className="bg-white py-24 md:py-32 border-b border-gray-100 overflow-hidden">
            <div className="container mx-auto px-6">
                <div className="flex flex-col lg:flex-row gap-12 items-center">
                            ...
                            <figure>
                                ...
                                <figcaption className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-full overflow-hidden">
                                        <img src={testimonialData.author.image} alt={testimonialData.author.name} className="w-full h-full object-cover" />
                                    </div>
                                    <div>
                                        <p className="font-bold text-lg">{testimonialData.author.name}</p>
                                        <p className="text-gray-500 text-sm">{testimonialData.author.role}</p>
                                    </div>
                                </figcaption>
                            </figure>
                        </Reveal>
                    ...
                    <div className="flex-1 w-full relative">
                        <Reveal delay={400} width="100%">
                            <div className="relative aspect-[4/3] bg-gray-100 rounded-lg overflow-hidden" role="img" aria-label={testimonialData.teamImage.alt}>
                                <img src={testimonialData.teamImage.src} alt="" className="w-full h-full object-cover" />
                ...
                <dl className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-16 pt-16 border-t border-gray-100">
                    {testimonialData.teamStats.map((item, index) => (
